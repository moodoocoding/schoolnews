-- One-time recovery after removing the collect-stage registry hard-fail
-- entirely (M22-COLLECT-REGISTRY-DRIFT-004). The 2026-08-14 run failed once
-- more at collect (INVALID_SOURCE_DATA) before that fix was deployed and
-- re-triggered. Remove only the exact 2026-08-14 terminal failure when it
-- has no durable topic, artifact, model or publish output. Any observable
-- work makes this migration fail closed.
...
begin
  ...
  if v_status is distinct from 'failed'
    ...
    raise exception using errcode = 'P0001', message = 'AUGUST14_POST_STALE_SOURCE_FIX_RESET_REFUSED';
  end if;
