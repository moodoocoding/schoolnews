-- One-time recovery after fixing the collect-stage source-registry guard
-- (M22-COLLECT-REGISTRY-GUARD-003). The 2026-08-14 run failed again at
-- collect (INVALID_SOURCE_DATA) because discovery-only rolling articles
-- introduced by 033 tripped a registry-consistency check that has since
-- been narrowed to evidence-bearing articles only. Remove only the exact
-- 2026-08-14 terminal failure when it has no durable topic, artifact,
-- model or publish output. Any observable work makes this migration fail
-- closed.
...
begin
  ...
  if v_status is distinct from 'failed'
    ...
    raise exception using errcode = 'P0001', message = 'AUGUST14_POST_COLLECT_GUARD_FIX_RESET_REFUSED';
  end if;
