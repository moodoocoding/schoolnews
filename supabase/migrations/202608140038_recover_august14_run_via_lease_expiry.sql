-- One-time recovery for 2026-08-14. Unlike 028/030/034-037, the run is now
-- terminal ('blocked') with a durable, immutable collect artifact already
-- attached (pipeline_artifacts_run_id_fkey forbids deleting the run row
-- while that artifact exists, and the artifact itself is immutable). Deleting
-- the row is therefore impossible; acquire_daily_run also treats any
-- terminal status as final regardless of lease expiry, so simply expiring
-- the lease would not help either.
--
-- Instead, put the run back into the exact shape acquire_daily_run already
-- knows how to resume: status 'running' with an expired lease and the score
-- step marked 'running' as an in-flight attemptNumber 1 (matching its
...
begin
  ...
  if not found then
    raise exception using errcode = 'P0001', message = 'AUGUST14_LEASE_RECOVERY_RUN_NOT_FOUND';
  end if;
  ...
  if v_row.status is distinct from 'blocked'
    ...
    raise exception using errcode = 'P0001', message = 'AUGUST14_LEASE_RECOVERY_REFUSED';
  end if;
  ...
  if v_score_step is null
    ...
    raise exception using errcode = 'P0001', message = 'AUGUST14_LEASE_RECOVERY_SHAPE_MISMATCH';
  end if;
  ...
  if (v_journal #>> '{revision}')::integer <> v_row.journal_revision then
    raise exception using errcode = 'P0001', message = 'AUGUST14_LEASE_RECOVERY_REVISION_DRIFT';
  end if;
  ...
  update news_clipping_private.daily_runs set
    ...
    journal = v_journal,
    finished_at = null,
    owner_id = 'august14-lease-recovery',
    lease_token = 'august14-lease-recovery-token',
