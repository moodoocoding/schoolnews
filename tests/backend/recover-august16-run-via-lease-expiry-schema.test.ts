...
const sql = readFileSync(
  "supabase/migrations/202608160042_recover_august16_run_via_lease_expiry.sql",
  "utf8",
).toLowerCase();

describe("august 16 lease-expiry recovery", () => {
  it("정확한 blocked/INVALID_SOURCE_DATA 실행만, 산출물이 없을 때만 되돌린다", () => {
    ...
    expect(sql).toContain("message = 'august16_lease_recovery_refused'");
  });
  ...
  it("score 단계 모양이 정확히 예상과 같을 때만 진행한다", () => {
    ...
    expect(sql).toContain("message = 'august16_lease_recovery_shape_mismatch'");
  });
