...
const sql = readFileSync(
  "supabase/migrations/202608140034_reset_august14_run_post_discovery_fix.sql",
  "utf8",
).toLowerCase();

describe("august 14 post-discovery-fix failure recovery", () => {
  it("내구 출력이 없는 정확한 실패 실행만 제거한다", () => {
    ...
    expect(sql).toContain(
      "message = 'august14_post_discovery_fix_reset_refused'",
    );
