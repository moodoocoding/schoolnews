...
const sql = readFileSync(
  "supabase/migrations/202608140038_recover_august14_run_via_lease_expiry.sql",
  "utf8",
).toLowerCase();

describe("august 14 lease-expiry recovery", () => {
  it("정확한 blocked/INVALID_SOURCE_DATA 실행만, 산출물이 없을 때만 되돌린다", () => {
    ...
    expect(sql).toContain("message = 'august14_lease_recovery_refused'");
  });
  ...
  it("score 단계 모양이 정확히 예상과 같을 때만 진행한다", () => {
    ...
    expect(sql).toContain("message = 'august14_lease_recovery_shape_mismatch'");
  });
  ...
  it("행을 삭제하지 않고 재개 가능한 running 상태로 되돌린다", () => {
    ...
    // journal.attempts entries require attemptNumber >= 1, so the
    // in-flight step must not be left at its original attemptNumber 0.
  ...
  it("journal.revision과 journal_revision 컬럼이 갱신 전후 일치하는지 확인한다", () => {
    ...
    expect(sql).toContain("message = 'august14_lease_recovery_revision_drift'");
  });
});
