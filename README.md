...
서비스는 매일 새벽 3시(KST)에 다음 작업을 자동으로 수행합니다.
...
- 매일 한국 표준시(`Asia/Seoul`) 새벽 3시에 후보 수집·평가를 실행합니다. Vercel Cron의 UTC 표현은 `0 18 * * *`입니다.
...
| 스케줄·배포 | Vercel Functions + Cron (`0 18 * * *`, KST 03:00) | 인증된 매일 평가와 최대 7일 발행 간격 |
...
매일 03:00 실행은 실행일 당일을 제외한 완료된 날짜만 봅니다. 평상시에는 전날 후보를 높은 즉시발행 기준으로 평가하고, 마지막 발행 후 7일째에는 DB에 누적된 직전 7일 기사·근거 전체를 동일한 결정론적 점수로 다시 평가합니다. 따라서 좋은 주제는 기다리지 않고 발행되며, 평범한 후보는 더 나은 기사를 기다릴 수 있습니다.
...
- 자동 실행: 매일 KST 03:00에 `collect → score → generate → validate → publish`를 실행합니다. 1~6일에는 전날의 높은 후보만 즉시 발행하고, 7일째에는 DB의 직전 7일 기사·근거를 다시 읽어 최고 적격 후보를 선택합니다. KST 날짜별 lease·fence·revision CAS, 모델 장부와 발행 영수증 복구 계보는 유지됩니다.
...
- Cron 목표 설정: `/api/cron/daily`는 `0 18 * * *`(UTC, KST 03:00)입니다. Vercel Hobby Cron은 지정 시각에서 지연될 수 있습니다. 인증 없는 요청은 HTTP 401로 차단됩니다.
...
운영 Cron은 매일 KST 03:00에 후보를 평가합니다. 좋은 후보는 바로 발행하고, 없으면 누적한 뒤 마지막 발행 후 7일째 최고 적격 후보를 선택합니다. 해외 기사, 출처·개인정보·생성 품질 기준을 우회한 글은 7일 마감에서도 발행하지 않습니다.
