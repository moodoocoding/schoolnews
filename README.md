...
- 실제 뉴스 수집: 국내 공식·연구 RSS 8개(AI타임스 포함), 네이버 뉴스 검색 API의 허용 매체 13개, 한국인공지능협회 공개 discovery API 1개까지 총 22개 소스에서 매일 제목·원문 링크·(가능한 경우) 공식 요약을 수집합니다. 검색 API 요약은 원문 전문으로 가장하지 않고 명시된 문장 범위 안에서만 근거 입력으로 사용합니다. 네이버 호스팅 페이지와 원문 페이지 직접 크롤링은 허용하지 않습니다. 네이버 소스는 12개 검색 쿼리를 쿼리 단위로 격리해 하나가 실패해도(타임아웃 15초 포함) 나머지 쿼리·소스는 정상 수집됩니다(M22-COLLECTOR-RELIABILITY-009).
...
- 통합 검증: ESLint 경고 0, TypeScript 통과, 105개 파일 576개 테스트와 프로덕션 빌드를 확인했습니다. 1일 즉시·7일 마감 경계, KST 날짜 창, 영속 과거 기사 조회, Naver API 요약 근거, 모델·출처·품질 fail-closed 경계를 검증했습니다.
...
- **해결 (M22-COLLECTOR-RELIABILITY-009, 배포 완료·마이그레이션 적용 대기)**: 대형 언론 13개(naver-summary-*)가 공유 Promise 하나로 묶여 있어 12개 검색 쿼리 중 하나만 실패해도 13개 소스 전체가 동시에 실패로 격리되던 문제를 쿼리 단위 격리로 교정했습니다(부분 실패는 해당 쿼리에서만 나온 소스만 실패로 정확히 좁힘, 15초 타임아웃 추가). 예외를 삼키던 범용 격리 로직에도 실제 에러명·코드를 남기도록 보강했습니다. 함께 AI타임스 공식 RSS와 한국인공지능협회 공개 discovery API를 신규 소스로 추가했습니다. `supabase/migrations/202608170045_aitimes_koraia_source_policies.sql`을 SQL Editor에서 적용해야 두 신규 소스가 실제로 수집을 예약할 수 있습니다.
