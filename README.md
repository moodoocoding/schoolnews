...
- 같은 `publisherGroupId`의 여러 피드는 독립 출처로 중복 계산하지 않습니다. RSS와 네이버 뉴스 검색 API의 짧은 요약은 평문으로 정제해 근거 후보로 만들고 각각 `RSS 요약`, `뉴스 검색 API 요약`으로 표시하며 직접 사실 권한은 `none`으로 강등합니다. 검색 결과는 기사 본문이 아니므로 passage 밖의 세부 사실을 만들 수 없습니다.
...
- 운영 수집은 과기정통부 공식 RSS와 네이버 뉴스 검색 API 메타데이터를 사용합니다. 네이버 검색은 `초등 AI 디지털 교육`, `교사 생성형 AI 교육`뿐 아니라 AI 에이전트·개인정보·저작권, 딥페이크·아동·청소년, 새로운 디지털 기술의 교육 영향을 찾는 제한된 질의를 하루 한 번 실행합니다. 동아일보·오마이뉴스·EBS뉴스·한국교육신문·전자신문·지디넷코리아·블로터·한겨레·경향신문·서울신문 등 검토된 국내 원문 도메인만 매체별로 분리하고, 연합뉴스·뉴시스는 통신 보조 출처로만 취급합니다. 기사 본문·이미지·유료 콘텐츠는 크롤링하지 않습니다.
