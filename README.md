...
AI·디지털 기술이 교육에 던지는 생각거리를 좋은 주제가 생기면 바로, 늦어도 7일 안에 한 편씩 소개하는 자동 뉴스 큐레이션 웹사이트입니다.
...
서비스는 매일 오전 9시(KST)에 다음 작업을 자동으로 수행합니다.

1. AI·디지털 교육과 관련된 최신 뉴스를 수집합니다.
2. 초등교육과의 관련성, 출처 신뢰도, 새로움 등을 기준으로 후보를 평가합니다.
3. 당일 주제 한 건과 관련된 여러 자료 및 과거 자료를 찾고, 이용 허락이 확인된 출처의 원문을 private 저장소에 보관합니다.
4. 선정 근거와 1:1로 연결된 원문을 Gemini에 전달해 짧은 게시물을 작성하고 대표 추상 비주얼을 준비합니다.
...
- 매일 한국 표준시(`Asia/Seoul`) 오전 9시에 후보 수집·평가를 실행합니다. Vercel Cron의 UTC 표현은 `0 0 * * *`입니다.
...
- 마지막 발행 후 1~6일에는 전날 기사 중 총점 82점·사회적 의미 6점 이상인 높은 품질 후보가 있을 때만 즉시 발행합니다.
- 즉시발행 후보가 없으면 기사와 근거를 private DB에 누적하고 공개 글은 만들지 않습니다.
- 마지막 발행 후 7일째에는 직전 7개 완료일의 모든 적격 후보를 다시 비교해 최고점 1건을 선택합니다.
- 7일 마감도 독립 출처·근거·개인정보·생성 품질 검사를 우회하지 않습니다. 안전하게 발행할 후보 자체가 없으면 내용을 꾸미지 않고 보류하며 운영 알림을 남깁니다.
...
- 원문 전체는 명시적으로 모델 분석 이용이 허용된 출처에 한해 짧은 보존기한으로 private schema에 저장하며 공개 화면·로그·artifact에는 포함하지 않습니다.
- 이용 허락이 없거나 불명확한 원문은 저장·모델 전송하지 않습니다. 제목만 보고 게시물을 생성하는 경로도 허용하지 않습니다.
...
| 일일 실행 | 순수 TypeScript `runDaily()` + CLI + 얇은 Cron 어댑터 | 매일 후보 평가와 수동 재실행 지원 |
| 스케줄·배포 | Vercel Functions + Cron (`0 0 * * *`, KST 09:00) | 인증된 매일 평가와 최대 7일 발행 간격 |
...
매일 09:00 실행은 실행일 당일을 제외한 완료된 날짜만 봅니다. 평상시에는 전날 후보를 높은 즉시발행 기준으로 평가하고, 마지막 발행 후 7일째에는 DB에 누적된 직전 7일 기사·근거 전체를 동일한 결정론적 점수로 다시 평가합니다. 따라서 좋은 주제는 기다리지 않고 발행되며, 평범한 후보는 더 나은 기사를 기다릴 수 있습니다.
...
- 모든 사실·맥락 문장은 근거 passage와 내부적으로 연결하고, 생성 시에는 각 선정 근거에 대응하는 원문 문서가 반드시 하나씩 있어야 합니다.
...
LLM은 제공된 근거 passage와 허용된 기사 원문을 쉬운 한국어로 재구성하는 역할만 담당합니다. 원문 문서는 기사 ID·근거 ID·출처·제목·발행시각과 1:1로 대조하며 SHA-256과 보존기한을 모델 호출 직전에 다시 검사합니다. 문서가 없거나 일부만 있거나, 변조·만료·학생 식별정보가 의심되면 모델 호출은 0회입니다. 현재 구현은 초안 1회와 필요 시 수정 1회, 최대 2회의 생성 호출만 허용합니다. 별도 모델 의미 평가도 같은 원문과 전체 호출·토큰·비용 상한 안에서만 실행합니다. 수정본이 다시 품질을 통과하지 못하면 발행을 보류합니다.
...
- 같은 `publisherGroupId`의 여러 피드는 독립 출처로 중복 계산하지 않습니다. 네이버 뉴스 검색은 제목·호스팅 기사 URL·발행일을 발견하는 단계까지만 담당합니다. 원문 수집은 별도의 출처별 허용 등록부, robots 규칙, 고정 Naver News origin·경로, 응답 형식·크기·로그인/유료벽 검사를 모두 통과할 때만 가능합니다. 현재 운영 허용 등록부는 비어 있어 네이버 원문 요청은 0건이며, 서면 허락이나 명확한 이용 조건을 확인하기 전에는 `discovery_only` 상태를 유지합니다.
...
- `generated-post-v7-fulltext`는 근거 catalog와 원문 문서를 분리된 JSON 데이터로 전달하고, 원문 안의 명령을 따르거나 제공되지 않은 사실을 추가하지 못하도록 명시합니다. 직접 교육 뉴스가 아닌 기술 뉴스에서는 확인된 기술 사실과 교육적 해석을 분리하고 영향은 가능성·긴장·질문으로만 표현합니다. 본문은 정확히 3문단·400~650자이며 450~550자를 권고합니다. 원문은 문서당 18,000, 전체 32,000 grapheme을 잘라내지 않고 상한 검사하며 권리 URL·해시는 모델에 보내지 않습니다.
...
| 2026-08-14 | M21-ADAPTIVE-CADENCE-001 | 최민재(루트) | 최민재 | 매일 KST 03:00에 후보를 수집·평가하고 높은 후보는 즉시 발행, 마지막 발행 후 7일째에는 누적 7일의 적격 후보 중 최고점을 선택하도록 발행 주기와 영속 조회 경계 개편 | `src/{db,lib,pipeline,repositories}/**`, `supabase/migrations/202608140024_*`~`025_*`, `tests/{backend,integration}/**`, `vercel.json`, `README.md`, `AGENTS.md` | 85 files / 523 tests, typecheck·lint·production build·diff-check 통과 | 024·025 운영 DB 적용과 Production 배포 전에는 기존 자정 정책 유지 |
...
| 2026-08-14 | M21-ARTICLE-REDISCOVERY-006 | 최민재(루트) | 최민재 | 동일 기사 재발견 시 최초 발견시각을 보존하고, 참조 없는 과거 `naver-news-*` 발견 행을 같은 URL의 `naver-summary-*` 근거 계보로 원자 전환하며, 출력 없는 실패 실행만 조건부 초기화 | `supabase/migrations/202608140029_*`~`031_*`, `tests/backend/{supabase-article-rediscovery,supabase-naver-discovery-migration,reset-rediscovery-failed-run}-schema.test.ts`, `README.md` | 기존 충돌 22건·evidence/topic/post 참조 0건 확인, 029 운영 적용·권한 확인, migration 정적 회귀·backend·typecheck·lint | 031 적용 후 복구 실행의 collect→score 이후 결과 확인 |
| 2026-08-14 | M21-ARTICLE-TEXT-REFRESH-007 | 최민재(루트) | 최민재 | 동일 URL·정규화 제목·내용 지문을 유지한 미사용 기사의 RSS 제목 문자 교정과 짧은 요약 갱신을 허용하되, 근거·주제·게시 계보가 생긴 기사는 계속 불변으로 차단 | `supabase/migrations/202608140032_*`, `tests/backend/supabase-article-text-refresh-schema.test.ts`, `README.md` | 운영 최근 7일 22건과 현재 재수집 결과를 필드별 대조해 보건복지부 5건의 텍스트 갱신을 확인, 정적 회귀·typecheck·lint | 032 적용 후 오늘 실행 복구 및 collect 결과 확인 |
| 2026-08-14 | M21-ROLLING-SNAPSHOT-008 | 최민재(루트) | 최민재 | collect 영속 범위를 실제 편집 대상인 완료된 최근 7일로 제한하고, 같은 기사가 피드에서 다시 발견되면 새 문구보다 이미 저장된 기사·근거 스냅샷을 우선해 근거 계보의 불변성을 유지 | `src/pipeline/orchestrator/supabase-daily-integration.ts`, `tests/integration/supabase-daily-integration.test.ts`, `README.md` | 7일 밖 항목 제외·동일 기사 피드 문구 변경 시 저장 스냅샷 우선 회귀 | Production 배포 후 오늘 실행 재개 확인 |
| 2026-08-14 | M21-PERSISTENCE-OBSERVABILITY-009 | 최민재(루트) | 최민재 | collect·score 영속 실패가 모두 같은 상위 오류로 축약되기 전에 기사·비밀·원격 원문 없이 저장소의 제한된 안정 오류 코드만 서버 로그에 기록 | `src/pipeline/orchestrator/supabase-daily-integration.ts`, `README.md` | lint·typecheck 및 운영 실패 재현 로그에서 원인 코드 확인 | 확인된 마지막 충돌 규칙 교정 |
| 2026-08-14 | M21-DISCOVERY-SNAPSHOT-010 | 최민재(루트) | 최민재 | 최근 7일 영속 스냅샷 조회가 근거 사용 출처뿐 아니라 허용된 discovery-only 기사도 반환하게 해, 같은 검색 기사의 재발견 시 저장된 식별정보를 재사용하고 `ARTICLE_IDENTITY_CONFLICT`를 방지 | `supabase/migrations/202608140033_*`, `tests/backend/supabase-discovery-editorial-materials-schema.test.ts`, `README.md` | migration 원자성·정확한 단일 필터 제거·service_role 전용 권한 회귀, typecheck·lint·diff-check 통과 | 033 운영 적용 후 실패 실행 초기화·Cron 재검증 |
| 2026-08-14 | M22-CODE-REVIEW-001 | 최민재(루트) | 정도현(자동화 신뢰성 리뷰) | 자동화 파이프라인 전체를 8앵글 코드 리뷰(정도현 프로필: 동시성·멱등성·트랜잭션 원자성 우선)로 검증해 correctness 결함 6건 수정 — ① generate 단계 lease 복구 시 이미 결제·저장까지 끝난 생성 결과가 재시도 상한에 막혀 영구 폐기되던 문제(모델-가능 단계에 한해 안전 재사용이 증명된 경우만 1회 보너스 시도 허용), ② 생성 폴백 라우트 성공이 직전 실패 라우트와 providerId가 다르다는 이유로 오탐 거부되던 문제, ③ 의미평가 폴백에서도 동일한 오탐 거부, ④ 생성 경로 감사가 의미평가 쪽에만 있던 미가격(unpriced) 호출 차단 규칙을 누락, ⑤ 실행 데드라인 판정이 서버 대신 클라이언트 시계를 사용해 시계 드리프트 시 조기 종료되던 문제(서버-클라이언트 오프셋 보정), ⑥ 8월 백필 재개 로직이 콘텐츠 동일성 검증 없이 날짜 존재만으로 건너뛰던 문제(발행 제목 대조로 복원). 부수적으로 provider/evaluator 원장 유틸(stableJson·recoveryRequired) 중복을 공용 모듈로 통합하고, discovery 마이그레이션 테스트에 accessStatus 필터 보존 회귀를 추가 | `src/pipeline/orchestrator/run-daily-pipeline.ts`, `src/pipeline/orchestrator/run-post-generation.ts`, `src/pipeline/orchestrator/ledgered-semantic-evaluator.ts`, `src/pipeline/generation/ledgered-generated-post-provider.ts`, `src/pipeline/generation/generation-support.ts`, `scripts/backfill-supabase-august-2026.ts`, `tests/backend/supabase-discovery-editorial-materials-schema.test.ts`, `README.md` | lint 0 경고, typecheck 통과, vitest 92개 파일 540개 테스트 통과, `next build` 프로덕션 빌드 성공 | 029~033 마이그레이션이 반복하는 문자열-앵커 patch 방식(재발견 시 필드별 가변성 규칙 미선언)은 이미 적용된 운영 마이그레이션의 재설계가 필요해 이번 범위에서 제외 — 별도 리팩터링 검토 필요. 수정된 생성/의미평가 폴백·lease 복구 경로는 실제 Gemini 이중 provider 구성이나 실제 lease 만료 상황에서는 아직 운영 재현 검증 전 |
| 2026-08-14 | M22-RESET-AUGUST14-002 | 최민재(루트) | 정도현 | 033 적용 전 재발생한 2026-08-14 `collect INVALID_SOURCE_DATA` 실패 실행을, 028·030과 동일한 안전 조건(내구 출력 0건일 때만)으로 다시 제거하는 일회성 migration 추가 | `supabase/migrations/202608140034_reset_august14_run_post_discovery_fix.sql`, `tests/backend/reset-august14-run-post-discovery-fix-schema.test.ts`, `README.md` | typecheck·lint·전체 vitest(93개 파일 541개 테스트) 통과, 원격 `get_daily_run` 조회로 실패 실행에 게시물·주제·artifact·모델 호출 산출물이 0건임을 사전 확인 | 033·034 운영 적용 후 Cron 재검증 |
| 2026-08-14 | M22-COLLECT-REGISTRY-DRIFT-004 | 최민재(루트) | 정도현 | 003 수정 배포 후에도 운영 Cron 재실행이 동일 코드로 다시 실패해 재조사: 실제 원인은 discovery-only 여부가 아니라, 근거가 실제로 연결된 과거 기사 중 `development-independent-news`/`development-public-authority`라는 개발 시드 잔여 출처가 현재 활성 출처 목록에 없다는 점이었음(로컬에서 RSS 7개·네이버 13개 실제 수집기를 직접 호출해 수집 자체는 정상임을 먼저 확인). `carryRollingMaterials`가 미등록 출처 기사를 이미 안전하게 걸러내므로, collect의 레지스트리 일치 하드 실패 자체를 제거하고 미등록 근거 출처는 `daily_collect_stale_evidence_source` 관측 로그만 남기도록 변경 | `src/pipeline/orchestrator/supabase-daily-integration.ts`, `tests/integration/supabase-daily-integration.test.ts`, `README.md` | typecheck·lint·전체 vitest(94개 파일 544개 테스트) 통과, 근거 연결된 폐기 출처 기사가 있어도 collect가 성공하는 회귀 테스트 추가 | Production 배포 후 실패 실행 재초기화(035)·Cron 재검증, `development-independent-news`/`development-public-authority` 잔여 시드 행 자체의 DB 정리 여부는 별도 검토 |
| 2026-08-14 | M22-COLLECT-REGISTRY-GUARD-003 | 최민재(루트) | 정도현 | 033 적용 후 운영 Cron 수동 재실행에서 collect가 다시 `INVALID_SOURCE_DATA`로 실패해 원인을 추적: 033이 `get_rolling_editorial_materials`에 근거 없는 discovery-only 과거 기사(레거시 `naver-news-*` 명명, 개발 시드 잔여 행 포함)까지 포함시키면서, collect의 "과거 출처가 모두 현재 활성 출처 목록에 있어야 한다"는 레지스트리 일치 검사가 그 비근거 행 때문에 전체 단계를 차단시킴. `carryRollingMaterials`가 활성 출처 밖 기사는 이미 안전하게 걸러내므로, 레지스트리 일치 검사 대상을 근거가 실제로 연결된(evidenceItems에 articleId가 있는) 과거 기사로만 좁힘 | `src/pipeline/orchestrator/supabase-daily-integration.ts`, `tests/integration/supabase-daily-integration.test.ts`, `README.md` | typecheck·lint·전체 vitest(93개 파일 542개 테스트) 통과, 신규 회귀 테스트로 근거 없는 미등록 출처 과거 기사가 있어도 collect가 성공함을 확인 | Production 배포 후 2026-08-14 실행 재초기화·Cron 재검증, 이 프로젝트 Supabase에 남아있는 `development-independent-news`/`development-public-authority` 같은 개발 시드 잔여 기사 행의 별도 정리 필요성 검토 |
| 2026-08-15 | M22-DAILY-RUN-DIAGNOSTICS-005 | 최민재(루트) | 정도현 | 004 배포 후 2026-08-14·15 운영 재실행에서 collect는 마침내 성공했으나, `score` 재개 시 "이미 succeeded인 단계 재사용 가능성" 검증이 `PIPELINE_VERSION_MISMATCH`로 막히는 두 번째, 별개의 실패를 발견. 근본 원인(설정 지문 자체는 로컬 재계산으로 프로덕션 저장값과 일치함을 확인)은 이번 세션에서 완전히 특정하지 못함. 원인 후보를 좁히기 위해 `asDailyPersistenceError`의 미분류 예외, `resolveStageInputFingerprint` 실패, 단계 재사용 검증(`inputFingerprint` 불일치 여부·`validateOutputReferenceWithin` 결과)에 안전한 진단 로그 3곳을 추가. `pipeline_artifacts`가 트리거로 완전히 불변(외래키 문제가 아니라 무조건 삭제 거부)임을 실제 운영 DB에서 확인해, 037/040의 "행 삭제 후 재시작" 접근이 collect 성공 이후로는 근본적으로 불가능함을 확정 | `src/pipeline/orchestrator/supabase-daily-integration.ts`, `src/pipeline/orchestrator/run-daily-pipeline.ts`, `supabase/migrations/202608150039_*`~`041_*`, `README.md` | typecheck·lint·전체 vitest(99개 파일 555개 테스트) 통과 | **다음 세션에서 마무리 필요**: 041 적용과 재실행 후 `daily_stage_reuse_check_failed`/`daily_stage_reuse_check_threw` 로그로 정확한 실패 사유 확인, 근본 수정, 2026-08-14(현재 `running`/`score` 상태로 방치, 정규 Cron은 항상 "오늘" 날짜만 재개하므로 자연 복구 안 됨)와 2026-08-15(현재 `blocked`/`PIPELINE_VERSION_MISMATCH`) 최종 정리 |
| 2026-08-16 | M22-DAILY-RUN-DIAGNOSTICS-006 | 최민재(루트) | 정도현 | 005의 진단 로그를 켠 채로 2026-08-16 정규 09:00 KST 실행을 직접 관찰: **한 번도 손대지 않은 완전히 새 실행에서도 collect 성공 직후 동일한 계열의 실패가 재현**되어, 이 버그가 수동 복구 절차의 부작용이 아니라 매일 반복되는 구조적 문제임을 확정. 로그(`daily_stage_reuse_check_failed collect true false 120000`)로 정확히 좁힘: collect 단계의 설정 지문(`fingerprintMatches`)은 항상 일치하지만, 방금 저장한 산출물을 다시 읽어 재검증하는 `validateOutputReferenceWithin`이 시간 초과 없이(120초 여유) 실패함. 원래 예외 자체는 `PipelineWorkspaceError`(score의 `resolveStageInputFingerprint` 실패 로그로 별도 확인)이지만 `referenceMatches`의 catch가 이를 삼켜 재사용 검증 쪽에서는 boolean으로만 보임. 즉 "쓸 때는 통과, 곧바로 다시 읽어 재검증하면 실패"하는 비대칭이 원인이며, Postgres jsonb 왕복 과정에서 payload 형태가 바뀌거나 재검증 로직 자체에 결함이 있을 가능성이 유력 — 정확한 필드는 미확정. `diagnosticErrorTag` 헬퍼로 두 진단 로그 모두 에러의 `code`까지 남기도록 강화 | `src/pipeline/orchestrator/run-daily-pipeline.ts`, `supabase/migrations/202608160042_*`, `README.md` | typecheck·lint·전체 vitest(100개 파일 558개 테스트) 통과, 실제 운영 재현으로 진단 로그 정확성 확인 | **다음 세션에서 이어서 진행**: `SupabasePipelineWorkspaceRepository#validateStoredRow`/`#validateCandidate`(shadow memory replay 검증)가 방금 쓴 `news_ingestion` payload를 왜 재검증에서 거부하는지 직접 재현(로컬에서 실제 collect 결과 payload를 저장→재조회하는 통합 테스트 작성 권장); 원인 확정 후 2026-08-14~16 세 날짜 모두 정리 필요 |

| 2026-08-16 | M22-CARRIED-EVIDENCE-CONTRACT-007 | 최민재(루트) | 정도현 | 006에서 좁힌 "쓸 때 통과·읽을 때 실패" 비대칭의 근본 원인을 프로덕션 payload 로컬 재현으로 확정: `newsIngestionResultSchema`가 "모든 후보의 evidenceIds 총합 = evidenceItems 개수"를 요구하는데, `carryRollingMaterials`가 최근 7일에서 이월한 근거(8/16 실행 기준 90건)는 이번 실행의 후보(20건)에 연결되지 않으므로 `0 ≠ 90`으로 항상 위반. 기사 개수 불변식에는 `carriedCount`가 반영돼 있었으나 근거 불변식에만 누락된 것이 원인. ① 근거 불변식을 이월 개념에 맞게 교정(후보 연결 근거는 실재·중복 없음을 유지하고, 이번 실행 수집 기사의 근거는 반드시 연결되도록 강제하되 이월 근거는 미연결 허용), ② 쓰기 경로(`createSupabasePipelineArtifactDescriptor`)에도 동일 런타임 계약 검증을 추가해 도메인 RPC 우회 경로의 비대칭 자체를 제거, ③ ②가 즉시 검출한 두 번째 위반(전 출처 실패 시 `historicalFallbackResult`의 `storage`가 `deduplicatedCount`와 불일치, 이월 자료가 있는데도 상태가 `failed`로 계산됨)도 함께 교정 | `src/repositories/memory-pipeline-workspace.repository.ts`, `src/repositories/supabase-pipeline-workspace.repository.ts`, `src/pipeline/orchestrator/supabase-daily-integration.ts`, `tests/backend/memory-pipeline-workspace.repository.test.ts`, `README.md` | lint 0 경고, typecheck 통과, 전체 vitest(101개 파일 563개 테스트) 통과, `next build` 성공. 이월 자료 왕복 회귀 테스트와 "이번 실행 근거 미연결은 계속 거부" 음성 테스트 추가. **프로덕션에 이미 저장된 2026-08-14·15·16 collect artifact 3건 모두 수정 후 재검증 통과를 읽기 전용으로 확인** | 배포 후 8/14~16 실행을 lease 복구 경로로 재개해 실제 발행까지 확인 |

| 2026-08-16 | M22-SCORE-RETRY-VERIFIED-008 | 최민재(루트) | 정도현 | 007 배포 후 2026-08-16 실행 재개를 시도하며 복구 절차 자체의 결함을 하나 더 교정하고, 파이프라인 정상 동작을 운영에서 실제 확인. 043은 score 단계를 `running`으로 되돌려 중단-단계 복구 경로를 태우려 했으나, 그 경로는 저널에 단계의 attemptNumber를 그대로 재기록하므로 이미 `score#1`(PIPELINE_VERSION_MISMATCH 실패)이 있는 상태에서는 "stage·attempt 중복"과 "attempt 번호 불연속"으로 `dailyRunJournalSchema`를 위반해, Cron이 DB를 건드리기도 전에 ZodError로 매번 중단됨. 044는 억지 복구 대신 파이프라인 자신의 재시도 표현(`failed_retryable` + 기존 1회차 메타데이터 재사용)으로 바꿔 정상 단계 루프가 score를 2회차로 실행하게 했고, `journal.attempts`는 손대지 않아 실제 첫 실패 감사 기록을 보존 | `supabase/migrations/202608160043_*`~`044_*`, `tests/backend/{recover-august16-run-after-contract-fix,retry-august16-score-after-contract-fix}-schema.test.ts`, `README.md` | 044 운영 적용 후 Cron 재실행에서 **`collect` 성공 → `score` 2회차 성공**으로 사흘간 막혀 있던 지점 통과 확인. 실행 결과는 `succeeded_without_publish`/`NO_ELIGIBLE_TOPIC`으로, 점수 기준을 통과한 후보가 없어 보류한 정상 동작이며 Gemini 호출 0회·비용 0 | 8/14·15는 지난 날짜라 정규 Cron 대상이 아니므로 그대로 두었음(필요 시 동일 절차로 복구 가능하나 후보 부족 보류가 유력). 마지막 발행이 8/13이므로 8/20이 7일 마감 — 누적 후보 중 최고점 선택이 실제로 동작하는지 그때 확인 |

| 2026-08-17 | M22-COLLECTOR-RELIABILITY-009 | 최민재(루트) | 정도현 | 사용자가 "왜 오늘도 수집이 20건밖에 안 되냐"고 물어 프로덕션 collect artifact를 읽기 전용으로 조사하면서 발견: 소스 20개 중 실제 대형 언론 13개(naver-summary-*)가 8/16·8/17 이틀 연속 전부 `SOURCE_UNAVAILABLE`로 실패해 소규모 관공서 RSS 7개만 남았던 것이 원인. 원인은 `collectNaverNewsSources`가 12개 검색 쿼리를 순차 실행하며 그중 하나만 실패해도 함수 전체가 reject되고, 13개 소스가 이 Promise 하나를 공유해 전부 동시에 실패로 격리되는 구조. 게다가 예외를 잡는 `collectIsolated`가 실제 에러를 버리고 "수집기 예외를 해당 수집원의 실패로 격리했습니다"라는 고정 문구만 남겨 원인 진단이 불가능했음. ① 쿼리별로 try/catch를 분리해 한 쿼리 실패가 나머지 11개와 13개 소스 전체를 막지 않도록 하고(부분 실패는 `partial`, 그 쿼리에서만 나온 소스는 `failed`로 정확히 구분), 쿼리별 15초 타임아웃을 추가(기존에는 무제한 대기 가능), 실패 사유에 실제 HTTP 상태·에러명을 담음 ② `run-news-ingestion.ts`의 범용 예외 격리 메시지에도 `diagnosticErrorTag`로 실제 에러명·코드를 덧붙임(방어적 이중 안전망). 사용자가 지목한 AI 전문지 4곳(더밀크·AI타임스·인공지능신문·한국인공지능협회)의 RSS/API 제공 여부도 직접 확인해 AI타임스 공식 RSS(`aitimes-com`)와 한국인공지능협회가 자사 페이지에서 공개 사용하는 Supabase 읽기 전용 API(`koraia-ai-news`, discovery_only)를 신규 소스로 추가. 인공지능신문은 RSS는 있으나 서버가 content-type을 `text/html`로 잘못 응답해 기존 수집기의 XML 허용 목록에 걸려 보류, 더밀크는 RSS/API가 없고 유료 구독(`free-article:false`, 별도 로그인 도메인) 콘텐츠라 크롤러를 만들지 않고 검토 기록에만 사유를 남김 | `src/pipeline/collectors/naver-news-collector.ts`, `src/pipeline/collectors/koraia-news-collector.ts`(신규), `src/pipeline/collectors/source-registry.ts`, `src/pipeline/collectors/index.ts`, `src/pipeline/orchestrator/run-news-ingestion.ts`, `src/lib/ops/configured-supabase-automation.ts`, `supabase/migrations/202608170045_*`, `tests/backend/{naver-news-collector,koraia-news-collector,rss-collector,expanded-source-policy-schema,aitimes-koraia-source-policy-schema}.test.ts`, `README.md` | lint 0 경고, typecheck 통과, 전체 vitest(105개 파일 576개 테스트) 통과, `next build` 성공. naver 쿼리 전체·부분 실패, 소스별 상태 분리, 타임아웃, koraia API 성공·실패 매핑을 신규/수정 테스트로 고정 | `supabase/migrations/202608170045_aitimes_koraia_source_policies.sql`을 프로덕션에 적용해야 신규 두 소스가 `SOURCE_POLICY_MISMATCH` 없이 수집을 예약할 수 있음(사용자가 SQL Editor에서 직접 실행 예정). 인공지능신문 content-type 완화 여부는 보류 상태로 남김 |

| 2026-08-17 | M22-AITIMES-KR-CONTENT-SNIFF-010 | 최민재(루트) | 정도현 | 사용자가 009에서 보류한 인공지능신문 수집을 요청. 서버가 실제로는 유효한 RSS XML을 `content-type: text/html`로 잘못 응답하는 것이 유일한 문제였으므로, 수집기 전체의 XML content-type 허용 범위를 넓히는 대신 `text/html`로 응답한 경우에 한해 본문을 먼저 읽고 시작부(`<?xml`/`<rss`/`<feed`/`<rdf`)로 실제 XML/RSS/Atom/RDF 여부를 확인한 뒤에만 통과시키도록 좁혀서 완화. 진짜 HTML 오류·리다이렉트 페이지는 `<!doctype`/`<html`로 시작하므로 이 조건으로 여전히 거부됨(회귀 테스트로 양쪽 모두 고정). 이 조건을 만족한 `aitimes-kr`을 정식 RSS 소스로 등록. 함께 요청받은 더밀크(themiilk.com)는 만들지 않기로 결정 — RSS/API가 없을 뿐 아니라 `sourceRegistryEntrySchema` 자체가 `collectionType: "html"`을 활성화하지 못하도록 이미 막고 있어(원문 크롤링 수집원 금지는 이 프로젝트의 기존 설계 원칙), 그 제약을 깨야 하는 데다 유료 구독 콘텐츠라 저작권·이용약관 문제도 있어 사용자에게 별도로 설명하고 보류 | `src/pipeline/collectors/rss-collector.ts`, `src/pipeline/collectors/source-registry.ts`, `src/lib/ops/configured-supabase-automation.ts`, `supabase/migrations/202608170046_aitimes_kr_source_policy.sql`, `tests/backend/{rss-collector,expanded-source-policy-schema,aitimes-kr-source-policy-schema}.test.ts`, `README.md` | lint 0 경고, typecheck 통과, 전체 vitest(106개 파일 580개 테스트) 통과, `next build` 성공. 실제 aitimes.kr 응답으로 로컬 재현해 확인한 뒤, "text/html이지만 유효 RSS는 수집" 양성 테스트와 "text/html이면서 진짜 HTML이면 여전히 거부" 음성 테스트를 모두 추가 | `supabase/migrations/202608170046_aitimes_kr_source_policy.sql` 프로덕션 적용 완료(사용자 SQL Editor에서 실행 확인). 더밀크는 사용자가 원할 경우 별도로 다시 논의 |

| 2026-08-17 | M22-DAILY-FORCE-CADENCE-011 | 최민재(루트) | 정도현 | 사용자가 "일단 매일 강제 발행을 해보고 그 후 5~7일 품질 관리로 전환"하고 싶다며, 기존 품질 우선 시스템은 그대로 둔 채 나중에 둘 중 하나를 고를 수 있는 스위치를 요청. `decidePublicationCadence`에 `mode?: "quality_gated" \| "daily_force"`를 추가(기본값 `quality_gated`는 기존 동작과 완전히 동일). `daily_force`는 매 실행을 "7일 마감" 모드처럼 취급해 7일 롤링 창의 최선 후보를 즉시-발행 전용 상한(총점 82·사회적 의미 6)에 얽매이지 않고 고릅니다. 다만 `buildCandidate`가 무조건 적용하는 기본 관련성 하한(총점 70·초등연관도 18·AI디지털특이성 10, `TOPIC_SELECTION_THRESHOLDS`)은 `daily_force`에서도 그대로 유지되므로, 이건 발행 주기만 앞당길 뿐 콘텐츠 관련성 기준을 낮추는 것은 아님 — 오늘(8/17) 실제 후보 중 1위였던 "통신 AI베팅"(38점, 초등연관도 0)처럼 관련성 하한을 못 넘는 날은 daily_force에서도 여전히 무발행일 수 있음을 사용자에게 명시. 환경변수 `PUBLICATION_CADENCE_MODE`(Vercel Production에서 `daily_force`로 설정)로 코드 재배포 없이 전환 가능하도록 구성 | `src/pipeline/orchestrator/publication-cadence.ts`, `src/pipeline/orchestrator/supabase-daily-integration.ts`, `src/lib/config/env.ts`, `src/lib/ops/configured-supabase-automation.ts`, `.env.example`, `tests/integration/publication-cadence.test.ts`, `README.md` | lint 0 경고, typecheck 통과, 전체 vitest(106개 파일 582개 테스트) 통과, `next build` 성공. daily_force의 강제 선택·7일 창·최초 실행 케이스를 신규 테스트로 고정. score 단계 fingerprint에 `cadenceMode`를 포함시켜 모드 전환 시 이전 캐시된 산출물을 재사용하지 않도록 함 | 사용자가 Vercel Production 환경변수에 `PUBLICATION_CADENCE_MODE=daily_force`를 추가하고 재배포까지 완료해 실제로 켜짐. 관련성 하한까지 낮춰 "무조건 매일" 발행을 원하는지는 아직 미정 — 며칠 운영해 보고 5~7일 품질 관리 모드(`quality_gated`)로 되돌릴지 판단 예정 |
...
- 실제 뉴스 수집: 국내 공식·연구·언론 RSS 9개(AI타임스·인공지능신문 포함), 네이버 뉴스 검색 API의 허용 매체 13개, 한국인공지능협회 공개 discovery API 1개까지 총 23개 소스에서 매일 제목·원문 링크·(가능한 경우) 공식 요약을 수집합니다. 검색 API 요약은 원문 전문으로 가장하지 않고 명시된 문장 범위 안에서만 근거 입력으로 사용합니다. 네이버 호스팅 페이지와 원문 페이지 직접 크롤링은 허용하지 않으며, `sourceRegistryEntrySchema`가 `collectionType: "html"` 자체를 활성화하지 못하게 막아 원문 HTML 크롤링 수집원은 구조적으로 추가할 수 없습니다(더밀크처럼 RSS·API가 없는 유료 매체는 이 때문에 보류). 네이버 소스는 12개 검색 쿼리를 쿼리 단위로 격리해 하나가 실패해도(타임아웃 15초 포함) 나머지 쿼리·소스는 정상 수집됩니다(M22-COLLECTOR-RELIABILITY-009). RSS 수집기는 `content-type`이 `text/html`이어도 본문이 실제 XML/RSS/Atom/RDF로 시작하면 수집하고, 진짜 HTML 오류 페이지는 여전히 거부합니다(M22-AITIMES-KR-CONTENT-SNIFF-010).
...
- 자동 실행: 매일 KST 09:00에 `collect → score → generate → validate → publish`를 실행합니다. `PUBLICATION_CADENCE_MODE=quality_gated`(기본값)에서는 1~6일에는 전날의 높은 후보만 즉시 발행하고 7일째에는 DB의 직전 7일 기사·근거를 다시 읽어 최고 적격 후보를 선택합니다. `daily_force`로 바꾸면 이 7일 로직을 매일 적용해 최선 후보를 강제 발행하되, 기본 관련성 하한은 그대로 유지됩니다. KST 날짜별 lease·fence·revision CAS, 모델 장부와 발행 영수증 복구 계보는 유지됩니다.
- 통합 검증: ESLint 경고 0, TypeScript 통과, 106개 파일 582개 테스트와 프로덕션 빌드를 확인했습니다. 1일 즉시·7일 마감 경계, KST 날짜 창, 영속 과거 기사 조회, Naver API 요약 근거, 모델·출처·품질 fail-closed 경계를 검증했습니다.
- Cron 목표 설정: `/api/cron/daily`는 `0 0 * * *`(UTC, KST 09:00)입니다. Vercel Hobby Cron은 지정 시각에서 지연될 수 있습니다. 인증 없는 요청은 HTTP 401로 차단됩니다.
...
- 알려진 제한: generate artifact가 이미 durable하게 저장된 뒤 lease가 만료되면(모델 호출·저장은 끝났지만 일일 실행 checkpoint 전에 워커가 중단된 경우), M22 코드 리뷰 수정 이후에는 저장된 산출물을 증명하는 재시도 1회를 허용해 재개합니다. 반면 finalize 후 artifact 저장 자체가 끝나기 전에 중단되면 여전히 중복 호출 대신 해당 날 실행을 안전 보류합니다(007 audit는 있지만 생성 본문 복구 저장소가 없음). 일시 장애로 terminal 실패가 된 같은 날짜 실행은 자동 재전송으로 다시 열지 않으므로 운영 알림과 승인된 복구 절차가 필요합니다. 공식 RSS가 늘어도 같은 사건의 독립 근거가 없거나 모델 입력이 허용된 원문이 없으면 무발행하는 것이 정상입니다. canonical survivor, 기사 단위 upstream provenance, DNS 재바인딩 방어, 알림, 접근성 자동 검사는 계속 남아 있습니다.
- **해결·운영 검증 완료 (M22-CARRIED-EVIDENCE-CONTRACT-007, M22-SCORE-RETRY-VERIFIED-008)**: 2026-08-14~16 사흘 연속 발행을 막았던 "collect 저장은 성공하는데 곧바로 재조회하면 실패"하는 문제는 `newsIngestionResultSchema`의 근거 불변식이 최근 7일 이월(`carriedCount`) 개념을 반영하지 못한 것이 원인이었습니다. 이월 근거는 이번 실행의 후보에 연결되지 않는데 스키마가 "후보 근거 총합 = 전체 근거 수"를 요구해 항상 위반됐고, collect의 쓰기가 도메인 RPC를 통해 이 검증을 우회하고 있었기 때문에 읽는 시점에야 드러났습니다. 불변식 교정과 함께 쓰기 경로에도 같은 검증을 추가해 비대칭 자체를 없앴습니다. 2026-08-16 운영 재실행에서 `collect` 성공 → `score` 성공으로 막혀 있던 지점을 실제로 통과했고, 그날은 점수 기준을 넘은 후보가 없어 `succeeded_without_publish`/`NO_ELIGIBLE_TOPIC`으로 정상 보류했습니다(모델 호출 0회). 이월 자료가 있는 collect 산출물의 저장→재조회 왕복은 회귀 테스트로 고정했습니다.
- **복구 절차에 관한 교훈**: 종료된 실행을 되살릴 때 단계를 `running`으로 되돌리면 중단-단계 복구 경로가 해당 단계의 `attemptNumber`를 저널에 그대로 재기록합니다. 그 단계에 이미 시도 기록이 있으면 "stage·attempt 중복"·"번호 불연속"으로 저널 계약을 위반해 실행이 시작조차 못 합니다. 이미 시도가 기록된 단계는 `failed_retryable`(기존 시도 메타데이터 유지)로 표시해 정상 재시도 경로를 태워야 하며, `journal.attempts`는 절대 다시 쓰지 않습니다.
- **해결 (M22-COLLECTOR-RELIABILITY-009, 배포·마이그레이션 적용 완료)**: 대형 언론 13개(naver-summary-*)가 공유 Promise 하나로 묶여 있어 12개 검색 쿼리 중 하나만 실패해도 13개 소스 전체가 동시에 실패로 격리되던 문제를 쿼리 단위 격리로 교정했습니다(부분 실패는 해당 쿼리에서만 나온 소스만 실패로 정확히 좁힘, 15초 타임아웃 추가). 예외를 삼키던 범용 격리 로직에도 실제 에러명·코드를 남기도록 보강했습니다. 함께 AI타임스 공식 RSS와 한국인공지능협회 공개 discovery API를 신규 소스로 추가했습니다.
- **해결 (M22-AITIMES-KR-CONTENT-SNIFF-010, 배포·마이그레이션 적용 완료)**: 인공지능신문(aitimes.kr)은 서버가 유효한 RSS XML을 `content-type: text/html`로 잘못 응답해 기존 수집기의 XML 허용 목록에 걸렸습니다. 수집기 전체의 허용 범위를 넓히는 대신 `text/html`로 응답한 경우에만 본문 시작부로 실제 XML/RSS/Atom/RDF 여부를 확인한 뒤 통과시키도록 좁혀서 해결했고, 진짜 HTML 오류 페이지는 여전히 거부됩니다. 더밀크(themiilk.com)는 RSS/API가 없고, 이 프로젝트가 처음부터 `collectionType: "html"`(원문 크롤링)을 스키마 수준에서 활성화 금지해 온 것과 유료 구독 콘텐츠라는 두 가지 이유로 크롤러를 만들지 않았습니다.
- **추가·활성화 완료 (M22-DAILY-FORCE-CADENCE-011)**: 기존 품질 우선 발행 주기는 그대로 두고, 환경변수 `PUBLICATION_CADENCE_MODE=daily_force`로 코드 재배포 없이 전환 가능한 "매일 최선 후보 강제 발행" 모드를 추가했습니다. 7일 마감 로직을 매일 적용하는 것과 같지만, `buildCandidate`의 기본 관련성 하한(총점 70·초등연관도 18·AI디지털특이성 10)은 이 모드에서도 그대로 유지되어 완전히 무관한 후보까지 발행하지는 않습니다 — 즉 "무조건 매일" 발행을 100% 보장하지는 않고, 발행 빈도만 크게 높입니다. 사용자가 Vercel Production에 이 환경변수를 설정하고 재배포까지 완료해 2026-08-18 09:00 KST 실행부터 적용됩니다. 며칠 운영해 보고 5~7일 품질 관리 모드로 되돌릴지 결정할 예정입니다.
...
운영 Cron은 매일 KST 09:00에 후보를 평가합니다. 좋은 후보는 바로 발행하고, 없으면 누적한 뒤 마지막 발행 후 7일째 최고 적격 후보를 선택합니다. 해외 기사, 출처·개인정보·생성 품질 기준을 우회한 글은 7일 마감에서도 발행하지 않습니다.
