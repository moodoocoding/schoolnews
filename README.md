...
- 본문: 3~5문단, 생성 목표 450~800자, 최대 900자. 결정론적 품질 게이트는 170자 미만의 카드형 단문을 `CONTENT_TOO_SHORT`로 보류하며, 근거가 부족할 때는 반복·추측으로 분량을 늘리지 않습니다.
- 질문: 1~2개, 질문당 최대 80자
...
- `generated-post-v3`는 짧은 근거 passage만 JSON 데이터로 전달하고, passage 안의 명령을 따르거나 외부 사실을 추가하지 못하도록 명시합니다. 본문은 배경·핵심 내용·교실과 가정에서의 의미가 이어지는 3~5문단을 요구합니다. 내부 기사 ID·해시와 원문 URL은 모델 입력에서 제외하며 이메일·전화번호는 제거합니다.
