...
- 자동 실행: 매일 KST 09:00에 `collect → score → generate → validate → publish`를 실행합니다. 1~6일에는 전날의 높은 후보만 즉시 발행하고, 7일째에는 DB의 직전 7일 기사·근거를 다시 읽어 최고 적격 후보를 선택합니다. KST 날짜별 lease·fence·revision CAS, 모델 장부와 발행 영수증 복구 계보는 유지됩니다.
- 통합 검증: ESLint 경고 0, TypeScript 통과, 106개 파일 580개 테스트와 프로덕션 빌드를 확인했습니다. 1일 즉시·7일 마감 경계, KST 날짜 창, 영속 과거 기사 조회, Naver API 요약 근거, 모델·출처·품질 fail-closed 경계를 검증했습니다.
