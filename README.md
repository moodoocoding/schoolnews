...
- **미해결 (2026-08-15 세션 종료 시점)**: collect 성공 후 score를 재개할 때 "이미 succeeded인 단계 재사용 가능" 검증이 `PIPELINE_VERSION_MISMATCH`로 막히는 원인 미상의 실패가 있습니다. 설정 지문 자체는 로컬 재계산과 저장값이 일치함을 확인했으나, `inputFingerprint` 비교와 `validateOutputReferenceWithin` 중 정확히 어느 쪽이 실패하는지는 아직 로그로 확인 전입니다(M22-DAILY-RUN-DIAGNOSTICS-005 참고). 이 때문에 2026-08-14는 `running`/`score` 상태로 방치되어 있고(정규 Cron은 항상 서버의 "오늘" 날짜만 재개하므로 과거 날짜는 자연 복구되지 않음), 2026-08-15는 `blocked`/`PIPELINE_VERSION_MISMATCH`로 종료된 상태입니다. `pipeline_artifacts`가 트리거로 완전히 불변(삭제 자체가 무조건 거부)이라 두 날짜 모두 "행을 지우고 처음부터 다시" 방식은 쓸 수 없으며, 041 migration을 적용하고 재실행해 새로 추가한 진단 로그를 확인하는 절차가 다음 세션에 필요합니다. 이 미해결 상태는 8/14·8/15 두 날짜의 발행 실패에만 영향을 주며, 2026-08-16 이후의 정상적인 새 실행(수동 복구 없이 처음부터 시작)에는 이 특정 코드 경로가 같은 방식으로 재현되지 않을 가능성이 있으나 확인 전입니다.
