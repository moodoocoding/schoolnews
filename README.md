...
- 통합 검증: ESLint 경고 0, TypeScript 통과, 101개 파일 563개 테스트와 프로덕션 빌드를 확인했습니다. 1일 즉시·7일 마감 경계, KST 날짜 창, 영속 과거 기사 조회, Naver API 요약 근거, 모델·출처·품질 fail-closed 경계를 검증했습니다.
...
- **해결 (M22-CARRIED-EVIDENCE-CONTRACT-007)**: 2026-08-14~16 사흘 연속 발행을 막았던 "collect 저장은 성공하는데 곧바로 재조회하면 실패"하는 문제는 `newsIngestionResultSchema`의 근거 불변식이 최근 7일 이월(`carriedCount`) 개념을 반영하지 못한 것이 원인이었습니다. 이월 근거는 이번 실행의 후보에 연결되지 않는데 스키마가 "후보 근거 총합 = 전체 근거 수"를 요구해 항상 위반됐고, collect의 쓰기가 도메인 RPC를 통해 이 검증을 우회하고 있었기 때문에 읽는 시점에야 드러났습니다. 불변식 교정과 함께 쓰기 경로에도 같은 검증을 추가해 비대칭 자체를 없앴으며, 프로덕션에 이미 저장된 세 날짜의 artifact가 모두 재검증을 통과함을 확인했습니다. 이월 자료가 있는 collect 산출물의 저장→재조회 왕복은 회귀 테스트로 고정했습니다.
