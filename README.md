...
- 통합 검증: ESLint 경고 0, TypeScript 통과, 94개 파일 544개 테스트와 프로덕션 빌드를 확인했습니다. 1일 즉시·7일 마감 경계, KST 날짜 창, 영속 과거 기사 조회, Naver API 요약 근거, 모델·출처·품질 fail-closed 경계를 검증했습니다.
...
- **미해결, 매일 반복되는 구조적 문제 (2026-08-16 세션 종료 시점)**: collect가 성공적으로 저장한 산출물을 곧바로 다시 읽어 재검증하면(`SupabasePipelineWorkspaceRepository`의 shadow memory replay 검증, `validateOutputReferenceWithin`/`resolveStageInputFingerprint` 경유) `PipelineWorkspaceError`로 실패합니다. 2026-08-14~16 세 날짜 모두, 수동 복구 여부와 무관하게(16일은 손대지 않은 완전히 새 실행) 동일하게 재현되어 매일 재발할 구조적 버그로 확정됐습니다. 설정 지문(configurationFingerprint) 자체는 로컬 재계산과 저장값이 정확히 일치함을 확인했으므로, "쓸 때는 통과하고 곧바로 다시 읽으면 실패"하는 비대칭이 원인이며 Postgres jsonb 왕복 중 payload 형태 변화나 재검증 로직 결함이 유력 후보입니다(정확한 필드는 미확정, M22-DAILY-RUN-DIAGNOSTICS-006 참고). 이 때문에 세 날짜 모두 게시물이 발행되지 못했고, `pipeline_artifacts`가 트리거로 완전히 불변이라 "행을 지우고 처음부터 다시" 방식도 쓸 수 없습니다. 다음 세션에서는 로컬 통합 테스트로 collect 산출물의 저장→재조회 왕복을 직접 재현해 정확한 실패 필드를 특정하고 수정하는 것이 최우선입니다 — 고치기 전까지는 이후 날짜의 자동 발행도 이 단계에서 계속 막힐 가능성이 높습니다.
